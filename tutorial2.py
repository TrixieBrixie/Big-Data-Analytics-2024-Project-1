...
def parallel_runner2(secs):
    ...
    for i in secs:
        ...
        processes.append(p)
    for p in processes:
        p.join() # The p.join() method is called on each process. 
                 # loop through the list to join each process, ensuring the main program waits for all processes to complete before proceeding. 
                 # This ensures that all processes finish before proceeding to the next line of code.
    end=time.perf_counter()
